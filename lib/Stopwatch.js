...
Stopwatch.prototype.suspend = function () {
    ...
    const delta = self.stop();
    self._incrementAccumulatedPauseTime(delta);
    return delta;
};
...
Stopwatch.prototype.resume = function () {
    ...
    self.setStartTimeDelta(self._accumulatedPauseTime);
...
Stopwatch.prototype._incrementAccumulatedPauseTime = function (pauseTime) {
    const self = this;

    self._accumulatedPauseTime += pauseTime;
};
